...
CREATE OR REPLACE PACKAGE BODY ArticlePackage AS
    ...
    BEGIN
        ...
        -- Utilisez la séquence pour obtenir l'ID auto-incrémenté
        -- Ajoutez des validations si nécessaire
        -- (Vérification de la plage de promo, etc.)
        -- INSERT INTO Article (nom, category, genre, couleur, description, prix, promo, disponible)
        -- VALUES (p_nom, p_category, p_genre, p_couleur, p_description, p_prix, p_promo, p_disponible);
    END InsertArticle;
    ...
    BEGIN
        ...
        -- SELECT AVG(notes) INTO v_average_note FROM Article WHERE id = p_id;
